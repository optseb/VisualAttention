        ...
        if (t >= lum->timeOn && t < lum->timeOff) {
            ...
            if (lum->shape == LUM_SHAPE_CROSS) {
                ...
                this->luminanceThetaMap.block (a, b, lum->thicknessBar, lum->widthThetaY) = MatrixXd::Constant (lum->thicknessBar,
                                                                                                                lum->widthThetaY,
                                                                                                                lum->brightness);
            } else if (lum->shape == LUM_SHAPE_CORNER) {
                // A corner with the position of the 'elbow' placed at thetaX/thetaY

                // First bar of corner. a,b gives centre.
                int a = lum->thetaX+WORLD_SPAN_OFFSET-(lum->thicknessBar/2);
                int b = lum->thetaY+WORLD_SPAN_OFFSET-(lum->thicknessBar/2);
                DBG ("Corner. a=" << a << " b=" << b << " widthThetaX:" << lum->widthThetaX << " widthThetaY:" << lum->widthThetaY << " thickness:" << lum->thicknessBar << " rotation:" << lum->rotation << " brightness:" << lum->brightness);
                if (lum->rotation == 90) { // 90 cw
                    this->luminanceThetaMap.block (a, b-lum->widthThetaX+lum->thicknessBar, lum->thicknessBar, lum->widthThetaX) = MatrixXd::Constant (lum->thicknessBar,
                                                                                                                                                       lum->widthThetaX,
                                                                                                                                                       lum->brightness);
                    this->luminanceThetaMap.block (a, b,                  lum->widthThetaY, lum->thicknessBar) = MatrixXd::Constant (lum->widthThetaY,
                                                                                                                                     lum->thicknessBar,
                                                                                                                                     lum->brightness);
                } else if (lum->rotation == 180) {
                    this->luminanceThetaMap.block (a-lum->widthThetaX+lum->thicknessBar, b, lum->widthThetaX, lum->thicknessBar) = MatrixXd::Constant (lum->widthThetaX,
                                                                                                                                                       lum->thicknessBar,
                                                                                                                                                       lum->brightness);
                    this->luminanceThetaMap.block (a, b-lum->widthThetaY+lum->thicknessBar, lum->thicknessBar, lum->widthThetaY) = MatrixXd::Constant (lum->thicknessBar,
                                                                                                                                                       lum->widthThetaY,
                                                                                                                                                       lum->brightness);

                } else if (lum->rotation == 270) {
                    this->luminanceThetaMap.block (a-lum->widthThetaY+lum->thicknessBar, b, lum->widthThetaY, lum->thicknessBar) = MatrixXd::Constant (lum->widthThetaY,
                                                                                                                                                       lum->thicknessBar,
                                                                                                                                                       lum->brightness);
                    this->luminanceThetaMap.block (a, b,                  lum->thicknessBar, lum->widthThetaX) = MatrixXd::Constant (lum->thicknessBar,
                                                                                                                                     lum->widthThetaX,
                                                                                                                                     lum->brightness);
            ...
            } else if (lum->shape == LUM_SHAPE_CHAR) {
                // A character. FIXME.
                int a = lum->thetaX+WORLD_SPAN_OFFSET-(lum->widthThetaX/2);
                int b = lum->thetaY+WORLD_SPAN_OFFSET-(lum->thicknessBar/2);
                DBG ("character. a=" << a << " b=" << b << " widthThetaX:" << lum->widthThetaX << " thickness:" << lum->thicknessBar);
                this->luminanceThetaMap.block (a, b, lum->widthThetaX, lum->thicknessBar) = MatrixXd::Constant (lum->widthThetaX,
                                                                                                                lum->thicknessBar,
                                                                                                                lum->brightness);
                a = lum->thetaX+WORLD_SPAN_OFFSET-(lum->thicknessBar/2);
                ...
                // rectangle shape.
                cout << "rectangle. thetaX:" << lum->thetaX << " WORLD_SPAN_OFFSET:" << WORLD_SPAN_OFFSET
                     << " lum->widthThetaX/2:" << (lum->widthThetaX/2) << " lum->brightness:" << lum->brightness << endl;

                if (lum->rotation == 270 || lum->rotation == 90) {
                    this->luminanceThetaMap.block (lum->thetaX+WORLD_SPAN_OFFSET-(lum->widthThetaX/2),
                                                   ...
                                                   lum->widthThetaY, lum->widthThetaX) = MatrixXd::Constant (lum->widthThetaY,
                                                                                                             lum->widthThetaX,
                                                                                                             lum->brightness);
                } else {
                    this->luminanceThetaMap.block (lum->thetaX+WORLD_SPAN_OFFSET-(lum->widthThetaX/2),
...
WorldFrame::oneDConvolution (const int& indexX, const int& indexY, const bool& xActive, Luminance& l)
{
    ...
    int activeWidth = 0;
    if (xActive) {
        DBG2 ("Convolution along thetaX with fixed indexY=" << indexY);
        indexActive = indexX;
        activeWidth = l.widthThetaX;
        indexFixed = indexY;
    } else {
        DBG2 ("Convolution along thetaY with fixed indexX=" << indexX);
        indexActive = indexY;
    ...
    while (n < this->luminanceThetaMap.cols() && n < indexActive + this->halfwidth + activeWidth) {
        ...
        if (xActive) {
            ...
            DBG2 ("xActive add to luminance [x:"<<l.thetaX<<",y:"<<l.thetaY<<",Ton:"<<l.timeOn<<",Toff:"<<l.timeOff<<"] blurLocations("<<l.bi<<",0=n="<<n<<"/1=m="<<m<<")");
            l.blurLocations (l.bi,0) = n;
            l.blurLocations (l.bi,1) = m;
        } else {
            this->luminanceThetaMap (m, n) = CR(n);
            DBG2 ("yActive add to [x:"<<l.thetaX<<",y:"<<l.thetaY<<",Ton:"<<l.timeOn<<",Toff:"<<l.timeOff<<"] blurLocations("<<l.bi<<",0=m="<<m<<"/1=n="<<n<<")");
            l.blurLocations (l.bi,0) = m;
            l.blurLocations (l.bi,1) = n;
        }
        ++n; ++l.bi;
    }
    DBG2 ("Final l.bi for this luminance at ("<<l.thetaX<< ","<< l.thetaY << ") is " << l.bi << " l.blurLocations.rows(): " << l.blurLocations.rows());
}
